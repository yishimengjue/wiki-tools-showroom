...
const categories = new Set(['interface-contract','capability-scope','control-flow','error-semantics','state-semantics','evidence-mismatch','pipeline-contract']);
...
for (const item of cases) {
  ...
  for (const field of ['title','repo','sourceFact','impact','skillFix']) if (typeof item[field] !== 'string' || !item[field]) throw new Error(item.id + ': missing ' + field);
...
for (const category of categories) if (cases.filter((item) => item.category === category && item.fromTable).length !== 1) throw new Error('Expected exactly one table representative: ' + category);
...
const data = {schemaVersion:1, reviewDate:'2026-09-07', cases, sourceSnippets};
